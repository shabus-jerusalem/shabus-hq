...
def main():
    with open(MEMBERS_CSV_FILENAME) as input_file:
        ...
        for member_row in reader:
            member_id, recommending_member_phone = process_row(member_row)
            recommending_member_phones[member_id] = recommending_member_phone

    for member_id, recommending_member_phone in recommending_member_phones.items():
        if recommending_member_phone == "":
            continue
        recommending_member = db.query.Member.query.get(phone_number=recommending_member_phone)
        db.query.Member.query.get(member_id).recommending_member = recommending_member
...
def process_row(member_row):
    ...
    member_dict = dict(zip(csv_columns, member_row))
    ...
    return recommending_member_phone_number
